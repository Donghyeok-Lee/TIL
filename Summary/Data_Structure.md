...
> 계층적 관계가 있는 데이터를 저장하는 자료구조
>
> 링크드 리스트와 유사하게 계층 관계가 있는 노드를 가리키는 레퍼런스를 지님

### 트리 용어 정리

- root 노드(뿌리 노드) : 트리의 시작 노드, 뿌리가 되는 노드

- 부모 노드 : 특정 노드의 직속 상위 노드
- 자식 노드 : 특정 노드의 직속 하위 노드 / 부모 노드와 반대
- 형제 노드 : 같은 부모를 갖는 노드
- leaf 노드 (잎/말단 노드) : 자식 노드를 갖고 있지 않은, 가장 말단에 있는 노드
- 깊이 : 특정 노드가 root 노드에서 떨어져 있는 거리
- 레벨 : 깊이 + 1. 깊이랑 거의 똑같은 개념
- 높이 : 트리에서 가장 큰 깊이
...
* 이진 트리
    ...
    * 전 이진 트리 (Full Binary Tree)
        ...
        * 모든 노드가 2 또는 0개의 자식을 갖는 이진 트리

    * 완전 이진 트리 (Complete Binary Tree)

        * 마지막 레벨을 제외한 모든 레벨에서 노드가 가득 차 있고,

            마지막 레벨에서는 왼쪽부터 노드가 채워진 트리

    * 포화 이진 트리 (Perfect Binary Tree)

        * 모든 레벨에 노드가 전부 채워진 이진 트리

### 트리 순회 방법

* 전위 순회 (pre-order)
    * 두 개의 부분 트리를 순회하기 **'전'**에 출력
    * 현재 노드 데이터를 출력 - 왼쪽 트리 순회 - 오른쪽 트리 순회
* 후위 순회 (post-order)
    * 두 개의 부분 트리를 순회한 **'후'**에 출력
    * 왼쪽 트리 순회 - 오른쪽 트리 순회 - 현재 노드 데이터를 출력
* 중위 순회 (in-order)
    * 부분 트리 순회 **'사이'**에 현재 노드 출력
    * 왼쪽 트리 순회 - 현재 노드 데이터를 출력 - 오른쪽 트리 순회



### 힙 (Heap)

> 1. 형태 속성 : 힙은 완전 이진 트리다.
>     * 마지막 레벨을 제외한 모든 노드가 채워지고, 마지막 레벨은 왼쪽부터 채워짐
>     * 노드의 개수가 n일 때, 높이는 O(lg(n))
...
> 저장된 데이터가 왼쪽 서브트리의 루트 <현재 노드 <오른쪽 서브 트리의 루트



## 그래프

> 연결 관계가 있는 연결 데이터를 저장할 수 있는 자료 구조

### 그래프 용어 정리

* 노드 : 그래프에서 하나의 데이터 단위를 나타내는 객체
* 엣지 : 그래프에서 두 노드의 직접적인 연결 관계 데이터
* 방향 그래프 (directed graph) : 엣지들이 방향을 가짐
* 가중치 그래프 (weighted graph) : 엣지들이 연결 관계뿐만 아니라 어떤 정보를 나타내는 수치를 가짐
* 차수 : 하나의 노드에 연결된 엣지들의 수
    * 무방향 그래프에서는 하나의 노드에 연결된 엣지들의 수
    * 방향 그래프에서는 출력 차수, 입력 차수로 구분함
...
* 인접 행렬 - 노드들 간의 연결 관계를 나타내는 2차원 배열
    * 무향 그래프 : 대칭행렬
    * 가중치 그래프 : 해당 위치에 가중치를 입력
    * 유향 그래프 : 비대칭 행렬
* 인접 리스트  노드들의 인접 데이터를 리스트에 저장

|                       |    인접 행렬     |            인접 리스트             |
| :-------------------: | :--------------: | :--------------------------------: |
|       공간 차지       | O(V<sup>2</sup>) |               O(V+E)               |
|       연결 확인       |       O(1)       |          O(V) / 선형 탐색          |
| 연결된 모든 노드 확인 | 전부 확인해야 됨 | 인접한 노드에 대한 레퍼런스만 존재 |